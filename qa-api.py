...
# Here we will add functionality to add the information dictionary.
# The new key is defined in the URL, and the value of the key is in the sent data.
# We also want to add a check for pre-existence, so that we do not update existing entires
...
def post_text(key):
    ...
    if key not in information:
        ...
        return Response(key + " added to information with value: " + request.data.decode('utf-8'),
                        mimetype='text/plain')
    else:
        return Response(key + " already exists.", mimetype='text/plain')

# We will implement update functionality (PUT request) with the same URL as the route for POST requests,
# but with a PUT method. Similar to before, we want to check the dictionary for pre-existence
# so that we only implement changes if the key already exists.
